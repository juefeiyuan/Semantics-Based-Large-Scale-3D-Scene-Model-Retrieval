...
def Net_Classifier(nfea=4096, nclasses=30):
    return nn.Linear(nfea, nclasses)

class Net_Prev_Pool(nn.Module):  
    def __init__(self, pool_idx=14):
...
class View_And_Pool(nn.Module):  
    ...
    def forward(self, x): 
        ...
        x = x.view(-1, 13, x.size()[1], x.size()[2], x.size()[3])
...
class Net_Whole(nn.Module):
    def __init__(self, nclasses=30, use_finetuned=False):
        super(Net_Whole, self).__init__()
        
        if use_finetuned:
            net = models.vgg11_bn(num_classes=30)   # 250 default
            #d = torch.load('/home/zczhou/Research/triplet_center_loss/finetune-sketch/vgg11_bn.pth.tar')
            d = torch.load('/D:/Projects/Journal2021/SceneSBR_IBR/models/vgg11_bn.pth.tar')
            sd = d['state_dict']
            od = net.state_dict()
...
if __name__ == '__main__':
    ...
    '''
    m = zzc_maxpooling()
    x = Variable(torch.rand(2, 13, 3, 224, 224).cuda())
    o = m(x)
    ipdb.set_trace()
